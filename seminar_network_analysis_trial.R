...
estimate_dcc_parameters <- function(dcc_inputs, target_C = NULL) {
  ...
  Z_mat <- as.matrix(dcc_inputs$residuals_dcc_initial)
  ...
  dcc_fit <- tryCatch({
    ...
    message("xdcclarge NL method failed: ", e$message)
    message("Falling back to manual DCC estimation with NL target...")
    NULL
  })
  
  if (!is.null(dcc_fit)) {
    # xdcclarge worked, extract parameters
    alpha_hat <- dcc_fit$a
    beta_hat  <- dcc_fit$b
    
    # Grab the target the package used if it exposes it, otherwise use ours
    if (!is.null(dcc_fit$C)) {
      target_used <- dcc_fit$C
    } else if (!is.null(target_C)) {
      target_used <- target_C
    } else {
      target_used <- create_target_matrix(Z_mat)
    }
    
    return(list(
      alpha = alpha_hat,
      beta  = beta_hat,
      target_C = target_used,
      method = "xdcclarge_NL"
    ))
  }
  ...
  if (is.null(target_C)) {
    target_C <- create_target_matrix(Z_mat)
  }
  ...
  opt <- optim(
    par = c(0.05, 0.93),
    fn = function(p) dcc_negloglik(p, Z_mat, target_C),
    method = "L-BFGS-B",
    lower = c(1e-6, 1e-6),
    upper = c(0.5, 0.999)
  )
...
run_dcc_network_monthly <- function(managed_portfolios,
  ...
  for (i in seq_along(month_map)) {
    ...
    if (need_estimation) {
      ...
      target_C <- tryCatch(
        create_target_matrix(dcc_inputs$residuals_dcc_initial),
        error = function(e) {
    ...
    Z_insample <- dcc_inputs$residuals_dcc_initial
